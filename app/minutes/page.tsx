...
export default function MinutesDashboard() {
  ...
  const [activeMenu, setActiveMenu] = useState('minutes');
  ...
  const fetchMeetings = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setAllMeetings(data);
        // Filter meetings that have minutes
        const meetingsWithMinutes = data.filter((m: any) => {
          if (!m.minutes?.discussions) return false;
          try {
            const parsed = JSON.parse(m.minutes.discussions);
            return parsed.savedMinutes && parsed.savedMinutes.length > 0;
          } catch {
            return false;
          }
        });
        setMeetings(meetingsWithMinutes);
        setFilteredMeetings(meetingsWithMinutes);
      }
    } catch (error) {
  ...
  const handleDownloadPDF = async (meetingId: string, meetingTitle: string) => {
    try {
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = `${meetingTitle.replace(/[^a-z0-9]/gi, '_')}_minutes.pdf`;
  ...
  const handleShareEmail = (meeting: any) => {
    ...
    let content = `Meeting Minutes: ${meeting.title}\n\nDate: ${new Date(meeting.date).toLocaleString()}\nLocation: ${meeting.location || 'N/A'}\n\n`;
    
    if (minutesData.attendees) {
      content += `Attendees: ${minutesData.attendees}\n\n`;
    }
    
    if (minutesData.minuteItems && minutesData.minuteItems.length > 0) {
      content += `Minutes:\n\n`;
      minutesData.minuteItems.forEach((item: any, i: number) => {
        content += `${i + 1}. ${item.topic || 'Item ' + (i + 1)}\n`;
        if (item.discussion) content += `   Discussion: ${item.discussion}\n`;
        if (item.decision) content += `   Decision: ${item.decision}\n`;
        if (item.actionItem) content += `   Action: ${item.actionItem}\n`;
        if (item.responsible) content += `   Responsible: ${item.responsible}\n`;
            ...
            {menuItems.map((item) => (
              <button
                ...
                onClick={() => {
                  ...
                  } else if (item.id === 'dashboard' || item.id === 'meetings' || item.id === 'reports' || item.id === 'profile') {
                    router.push('/user');
        ...
        <header className="bg-white border-b border-gray-200 sticky top-0 z-30 shadow-sm">
          <div className="flex items-center justify-between px-6 py-4">
            ...
            <button
              ...
              className="px-6 py-2.5 bg-blue-400 hover:bg-blue-500 text-white font-medium rounded-lg transition-colors shadow-sm ml-4"
            >
              + New Minutes
            </button>
          </div>
        ...
        <div className="p-6">
          {filteredMeetings.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-2xl shadow-sm border border-gray-200">
              ...
              <p className="text-gray-400 mb-4">{searchQuery ? 'No minutes found' : 'No minutes yet'}</p>
              {!searchQuery && (
                <button
                  onClick={() => setShowCreateModal(true)}
                        ...
                        <div className="flex items-center gap-4 text-sm text-gray-500">
                          ...
                          <span className="flex items-center gap-1">
                            ...
                            {(() => {
                              try {
                                const parsed = JSON.parse(meeting.minutes.discussions);
                                const items = parsed.savedMinutes?.[parsed.savedMinutes.length - 1]?.minuteItems || [];
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-gray-900/50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            ...
            <p className="text-sm text-gray-600 mb-4">Choose a meeting to create minutes for:</p>
