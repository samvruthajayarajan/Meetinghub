      ...
      <main className="flex-1 overflow-y-auto bg-white">
          ...
          <div className="p-6">
            ...
            {filteredMeetings.length === 0 ? (
              <div className="text-center py-12 bg-white rounded-3xl shadow-sm border border-gray-200">
                ...
                <p className="text-gray-400 mb-4">{searchQuery ? 'No meetings found' : 'No meetings yet'}</p>
                {!searchQuery && (
                  <button
                    onClick={() => router.push('/meetings/new')}
                    className="px-6 py-3 bg-green-100 text-green-700 rounded-xl hover:bg-green-200 transition-colors shadow-sm font-medium"
                  ...
                  {meetings.map((meeting) => (
                    ...
                    >
                      <div className="flex flex-col gap-3">
                        {/* Meeting Info */}
                        <div className="flex-1 min-w-0">
                          <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-2 truncate">{meeting.title}</h3>
                          <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs sm:text-sm text-gray-500 mb-2">
                            <span className="flex items-center gap-1">
                              <svg className="w-3 h-3 sm:w-4 sm:h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                              </svg>
                              <span className="truncate">{new Date(meeting.date).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })} at {new Date(meeting.date).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}</span>
                            </span>
                            <span className="flex items-center gap-1">
                              <svg className="w-3 h-3 sm:w-4 sm:h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            onClick={async (e) => {
                              ...
                              try {
                                ...
                                if (response.ok) {
                                  ...
                                  a.href = url;
                                  a.download = `${meeting.title.replace(/[^a-z0-9]/gi, '_')}_minutes.pdf`;
                ...
                >
                  ...
                  <div className="space-y-2">
                    <button
                      ...
                      className="w-full px-4 py-2 bg-blue-100 text-blue-700 rounded-xl hover:bg-blue-200 transition-colors text-sm font-medium flex items-center justify-center gap-2"
                    >
        ...
        {activeMenu === 'profile' && (
          <div className="p-4 sm:p-6">
            ...
            <div className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-sm border border-black max-w-2xl">
              <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 mb-4 sm:mb-6 pb-4 sm:pb-6 border-b border-gray-800">
                <div className="w-16 h-16 sm:w-20 sm:h-20 bg-black rounded-full flex items-center justify-center text-white font-bold text-xl sm:text-2xl shadow-md flex-shrink-0">
                  {session?.user?.name?.charAt(0).toUpperCase()}
                </div>
                <div className="flex-1 text-center sm:text-left">
                  <h2 className="text-lg sm:text-xl font-semibold text-gray-800">{session?.user?.name}</h2>
                  <p className="text-sm sm:text-base text-gray-600 break-all">{session?.user?.email}</p>
                  <span className="inline-block mt-2 px-3 py-1 bg-gray-800 text-white rounded-full text-xs font-medium">
                ...
                {!isEditingProfile && (
                  <button
                    ...
                    className="w-full sm:w-auto px-4 py-2 bg-gray-800 text-white rounded-xl hover:bg-black transition-colors font-medium flex items-center justify-center gap-2 text-sm sm:text-base"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                    </svg>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-3 sm:px-4 py-2 sm:py-3 border rounded-xl text-sm sm:text-base text-gray-800 ${
                      isEditingProfile 
                        ? 'bg-white border-gray-800 focus:ring-2 focus:ring-black focus:border-transparent'
                ...
                <div>
                  ...
                  <input
                    ...
                    disabled
                    className="w-full px-3 sm:px-4 py-2 sm:py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm sm:text-base text-gray-800 break-all"
                  />
                ...
                {isEditingProfile && (
                  <div className="flex flex-col sm:flex-row gap-3 pt-2">
                    <button
                      ...
                      className="w-full sm:flex-1 px-4 py-3 bg-gray-200 text-gray-700 rounded-xl hover:bg-gray-300 transition-colors font-medium text-sm sm:text-base"
                    >
                      Cancel
                    </button>
                    <button
                      ...
                      disabled={updating}
                      className="w-full sm:flex-1 px-4 py-3 bg-black text-white rounded-xl hover:bg-gray-800 transition-colors font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-sm sm:text-base"
                    >
                      {updating ? (
                        <>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <p className="text-gray-900">{new Date(selectedMeeting.date).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}</p>
              </div>

              <div>
                <p className="text-sm text-gray-500 mb-1">Time:</p>
                <p className="text-gray-900">{new Date(selectedMeeting.date).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}</p>
              </div>

              <div>
                <p className="text-sm text-gray-500 mb-1">Location:</p>
                <p className="text-gray-900">{selectedMeeting.location || 'N/A'}</p>
              </div>

              <div>
                <p className="text-sm text-gray-500 mb-1">Meeting Mode:</p>
                <p className="text-gray-900 capitalize">{selectedMeeting.meetingMode || 'offline'}</p>
              </div>
              ...
              <div>
                ...
                <p className="text-gray-900 capitalize">{selectedMeeting.status?.toLowerCase() || 'scheduled'}</p>
              </div>

              <div>
                <p className="text-sm text-gray-500 mb-1">Attendees:</p>
                <p className="text-gray-900">{selectedMeeting.attendees?.join(', ') || selectedMeeting.participants?.join(', ') || 'N/A'}</p>
              </div>
            ...
            <div className="p-6 border-t border-gray-200 flex gap-3">
              <button
                ...
                className="flex-1 px-6 py-3 bg-blue-400 hover:bg-blue-500 text-white font-medium rounded-lg transition-colors"
              >
                Edit Meeting
              </button>
              <button
